...
describe('#Revocation Rules Sync', function RevocationRulesSyncSuite() {
  ...
  it('Should be able to sync rules', async function test() {
    ...
    strictEqual(revocationRulesStorage.getRules().length, 0);

    await kv.set(`${KEY_PREFIX_REVOCATION_RULES}001`, 'i am rule 001');
    await kv.del(`${KEY_PREFIX_REVOCATION_RULES}001`);
    await kv.set(`${KEY_PREFIX_REVOCATION_RULES}002`, 'i am rule 002');
    await kv.set(`${KEY_PREFIX_REVOCATION_RULES}003`, 'i am rule 003');
    await kv.set(`${KEY_PREFIX_REVOCATION_RULES}003`, 'i am rule 003 updated');
    await kv.set(`${KEY_PREFIX_REVOCATION_RULES}003`, 'i am rule 003 updated once again');

    // await for the last key sync before stopping watching
    await Bluebird.delay(100);

    const rules = this.users.revocationRulesStorage.getRules();
    strictEqual(rules.length, 2);
    strictEqual(rules[0].Key, 'revocation-rules/002');
    strictEqual(rules[0].Value, 'i am rule 002');
    strictEqual(rules[1].Key, 'revocation-rules/003');
